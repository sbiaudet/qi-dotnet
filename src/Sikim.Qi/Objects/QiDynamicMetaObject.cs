...
{
    public class QiMetaObject : DynamicMetaObject
    {
        public QiMetaObject(Expression expression, object value) : base(expression, BindingRestrictions.Empty, value) { }
        ...
        {
            Collection<Expression> paramArgs = new Collection<Expression>();
        ...
        {
            Collection<Expression> block = null;
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                {
                    if (block == null)
                        block = new Collection<Expression>();
        ...
        {
            var methods = Value.GetType().GetMembers(BindingFlags.Public | BindingFlags.Instance).Where(m => m.Name == method && m.MemberType == MemberTypes.Method).ToArray() ;
        ...
        {
            var valueType = RuntimeType ?? Expression.Type;
